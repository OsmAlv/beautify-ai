...
export async function POST() {
  ...
  for (const update of updates) {
    ...
    if (environment === null) {
      query = query.is('environment', null);
    } else {
    ...
    if (error) {
      ...
      results.push({ status: 'error', model, intensity, environment, error: error.message });
    } else {
      console.log(`✅ Обновлен промпт: ${model}/${intensity}/${environment || 'null'}`);
      results.push({ status: 'success', model, intensity, environment });
    }
  }

  return NextResponse.json({ 
    message: 'Промпты обновлены', 
    results 
  });
}
